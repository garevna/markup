...
У встроенных объектов, которые не являются конструкторами ( типа *Math* ) цепочка протипов будет короче - свойство **`__proto__`** будет ссылкой на **`Object`**

#### :briefcase: **Задание**: выведите в консоль все нативные встроенные объекты и отследите в цепочке прототипов ссылку на **`Object`**

<a name="constructor"></a>
## 🎓 Конструктор

| [:arrow_heading_up:](#proto) | <img width="800"/> | [:arrow_heading_down:](#Object) |
|-|-|-|

Настало время разобраться с тем, что же такое конструктор

:white_square_button: это функция, создающая экземпляры объектов с заданным набором свойств
...
```
    ...
    Его цепочка прототипов содержит два "звена"  

:white_square_button: конструктор ничего не возвращает с помощью оператора return ( вставить-то его можно, но зачем? ) 

:white_square_button: Имена конструкторов принято начинать с заглавной литеры

`потому что по-сути имя конструктора - это имя класса`
...
☝ Все свойства, объявленные в конструкторе с ключевым словом  var или function,
будут приватными  ( defaultName, showName )
...
☝ При вызове конструктора обязательно нужно использовать ключевое слово new

В публичных методах экземпляра  this  будет ссылкой на контект вызова
( экземпляр, в контексте которого вызван метод )

     var lena = new Girl ( "Лена", 18 )
     lena.changeName

⚡ в приватных методах экземпляра контекстом вызова будет 
