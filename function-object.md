...
<table>
    <tr>
        <td width="500"><a href="#arguments">▶️</a><code> Объект <strong>arguments</strong></code></td>
    ...
    <tr>
        <td><a href="#execution-context">▶️</a><code> Контекст выполнения</code></td>
    </tr>
    <tr>
...
Для обычной функции контекстом вызова является глобальный объект **`window`**

`Все, что не находится в частной собственности, по умолчанию является собственностью ♚ ( глобального объекта )`

`⚠️ Поэтому при вызове обычной функции указывать window перед ее именем не нужно - это подразумевается по умолчанию`
